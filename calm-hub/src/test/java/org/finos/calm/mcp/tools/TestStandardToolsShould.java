...
class TestStandardToolsShould {
    ...
    void return_standards_when_namespace_has_entries() throws NamespaceNotFoundException {
        when(standardStore.getStandardsForNamespace("finos"))
                .thenReturn(List.of(
                        new NamespaceResourceSummary("Security Controls", "Defines security requirements", 1, 0)
                ));
    ...
    void create_standard_version_successfully() throws Exception {
        ...
        when(standardStore.getStandardsForNamespace("finos"))
                .thenReturn(List.of(new NamespaceResourceSummary("My Standard", "A description", 1, 0)));
    ...
    void return_error_when_standard_not_in_summary_does_not_overwrite_metadata() throws Exception {
        ...
        when(standardStore.getStandardsForNamespace("finos"))
                .thenReturn(List.of(new NamespaceResourceSummary("Other Standard", "other desc", 2, 0)));
    ...
    @Test
    void reject_oversized_json_for_create_standard() {
        String hugeJson = "{\"x\":\"" + "a".repeat(100_001) + "\"}";
        ToolResponse result = standardTools.createStandard("finos", "name", "desc", hugeJson);

        assertThat(result.isError(), is(true));
        assertThat(text(result), containsString("Standard JSON"));
        verifyNoInteractions(standardStore);
    }

    @Test
    void reject_oversized_json_for_create_standard_version() {
        String hugeJson = "{\"x\":\"" + "a".repeat(100_001) + "\"}";
        ToolResponse result = standardTools.createStandardVersion("finos", 1, "1.1.0", hugeJson);
    ...
    void create_standard_version_preserves_existing_name_and_description() throws Exception {
        when(standardStore.getStandardsForNamespace("finos"))
                .thenReturn(List.of(new NamespaceResourceSummary("Original Name", "Original Desc", 1, 0)));
        ...
        standardTools.createStandardVersion("finos", 1, "1.1.0", "{}");

        ArgumentCaptor<CreateStandardRequest> captor = ArgumentCaptor.forClass(CreateStandardRequest.class);
        verify(standardStore).createStandardForVersion(captor.capture(), eq("finos"), eq(1), eq("1.1.0"));
        assertThat(captor.getValue().getName(), is("Original Name"));
        assertThat(captor.getValue().getDescription(), is("Original Desc"));
    }
}
