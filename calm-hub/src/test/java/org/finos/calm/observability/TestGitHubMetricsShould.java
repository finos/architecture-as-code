...
class TestGitHubMetricsShould {
    ...
    @Test
    void record_session_decrypt_success() {
        metrics.recordSessionDecryptSuccess();
        metrics.recordSessionDecryptSuccess();

        Counter counter = registry.find("calm.github.session.decrypt").tag("outcome", "success").counter();
        assertThat(counter, is(notNullValue()));
        assertThat(counter.count(), equalTo(2.0));
    }

    @Test
    void record_session_decrypt_failure() {
        metrics.recordSessionDecryptFailure();

        Counter counter = registry.find("calm.github.session.decrypt").tag("outcome", "failure").counter();
        assertThat(counter, is(notNullValue()));
        assertThat(counter.count(), equalTo(1.0));
    }
