...
/**
 * Pattern-specific wiring only — the fan-out is {@link NitriteVersionSplitMigration},
 * exercised in depth by {@code TestNitriteArchitectureVersionSplitStepShould}.
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
class TestNitritePatternVersionSplitStepShould {

    private NitriteCollection headers;
    private NitriteCollection versions;
    private NitritePatternVersionSplitStep step;
    ...
    void setup() {
        ...
        when(db.getCollection("patterns")).thenReturn(headers);
        when(db.getCollection("patternVersions")).thenReturn(versions);

        step = new NitritePatternVersionSplitStep(db);
    ...
    void fan_a_namespace_document_out_into_pattern_headers_and_versions() {
        stubCollectionContents(List.of(Document.createDocument()
                .put("namespace", "finos")
                .put("patterns", List.of(Document.createDocument()
                        .put("patternId", 1)
                        .put("name", "Sample")
        ...
        assertThat(headerCaptor.getValue().get("patternId", Integer.class), is(1));
