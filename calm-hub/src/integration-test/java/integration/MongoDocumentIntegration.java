...
class MongoDocumentIntegration {
    ...
    @BeforeAll
    static void setup() {
        String uri = ConfigProvider.getConfig().getValue("quarkus.mongodb.connection-string", String.class);
        String databaseName = ConfigProvider.getConfig().getValue("quarkus.mongodb.database", String.class);
    ...
    void lists_no_documents_initially() {
        given().get("/api/calm/namespaces/finos/documents/pattern").then().statusCode(200).body("values", empty());
    }

    @Test @Order(2)
    void creates_a_document() {
        String location = given().contentType("application/json").body(request("Mongo document", MARKDOWN))
    ...
    void creates_and_orders_immutable_versions() {
        ...
        given().contentType("application/json").body(request("Mongo document v2", versionTwo))
        ...
        given().contentType("application/json").body(request("Mongo document v2", versionTwo))
                ...
                .then().statusCode(201);
        given().contentType("application/json").body(request("Mongo document v3", versionTwo))
