...
/**
 * Captures an {@link AuditLogEntry} for every mutating (POST/PUT/DELETE) request that completes
 * with a {@code SUCCESS} (2xx) or {@code DENIED} (401/403) outcome.
 *
 * <p>Deliberately a plain {@link ContainerResponseFilter} — <strong>not</strong>
 * {@code @PreMatching} like {@link ReadOnlyRequestFilter} — because it needs {@link
 * UriInfo#getPathParameters()} resolved, which only happens after JAX-RS routing/resource-matching.
 * Running post-matching also means this filter observes the final response, including responses
 * produced by a {@code @PermissionsAllowed} denial (a thrown security exception mapped to 401/403),
 * so denied mutation attempts are captured with no less fidelity than successful ones for endpoints
 * that already carry an existing-entity path parameter.
 *
 * <h2>How entity identity is resolved, without per-endpoint code</h2>
 *
 * <ol>
 *   <li><b>Staged audit context</b> — a small number of endpoints where the entity's identity can
 *       only be determined from the request body, not the URL, stage an {@link AuditContext} via
 *       {@link #stage} before their permission check runs (see {@code
 *       NamespaceResource#createNamespace} and {@code MappingControllerResource}'s two generic
 *       {@code /calm} write methods). When present, it is used verbatim and no further resolution
 *       happens.
 *   <li><b>Path parameters</b> — nearly every other mutating endpoint already encodes {@code
 *       namespace}/{@code domain} + the entity's own ID + {@code version} as JAX-RS path template
 *       parameters, so this filter reads them directly.
 *   <li><b>{@code Location} response header</b> — for server-generated-ID create endpoints (the
 *       entity's own ID isn't in the request path at all), every one of them returns {@code
 *       Response.created(URI)} on success with a {@code Location} that encodes the new entity's
 *       identity. {@link LocationSegmentParser} extracts it. This only applies to {@code SUCCESS}
 *       outcomes — on {@code DENIED}, no store call (and thus no {@code Location}) ever happens, so
 *       {@code entityId} is {@code null} for the small number of endpoints that are both
 *       server-generated-ID and denied via a {@code @PermissionsAllowed} interceptor (which rejects
 *       before the method body runs, so no hook could stage a context either). This is an accepted,
 *       narrow gap — see the design discussion referenced from the audit logging feature proposal.
 * </ol>
 *
 * <h2>{@code action} derivation — a documented simplification</h2>
 *
 * A {@code POST} to a path that already names an existing entity's own ID (e.g. {@code POST
 * .../architectures/{architectureId}/versions/{version}}) is labeled {@code UPDATE}, even when that
 * call happens to create the very first version of a brand-new resource. Distinguishing "first
 * version" from "Nth version" from path shape alone isn't reliable without a store round-trip this
 * filter deliberately avoids doing. {@code entityId}/{@code version} retain full fidelity
 * regardless — only the coarse {@code action} label is affected.
 *
 * <h2>Read-only mode</h2>
 *
 * No special-casing is needed: {@code calm.readonly=true} installs {@link ReadOnlyRequestFilter} at
 * {@code @PreMatching @Priority(0)}, which rejects every mutating verb with 405 before routing —
 * before this (post-matching) filter would ever see the request. 405 is also not in the
 * SUCCESS/DENIED status set this filter records, so no mutation can ever reach this filter's
 * recording logic in read-only mode.
 *
 * <h2>{@code no-auth} mode</h2>
 *
 * {@link #resolveActor()} returns {@code identity.getPrincipal().getName()} for any non-anonymous
 * identity. In {@code no-auth} mode every caller is granted the same trusted, non-anonymous
 * principal literally named {@code "no-auth"} ({@code NoAuthAuthenticationMechanism}), so every
 * audit record's {@code actor} field is that shared literal, not a real per-caller username — a
 * known limitation of that deployment mode this filter cannot fix. {@link AuditService#init()} logs
 * a one-time startup warning about this when audit recording is enabled alongside {@code no-auth}.
 */
@ApplicationScoped
@Provider
public class AuditRequestFilter implements ContainerResponseFilter {
    ...
    /**
     * A pre-resolved audit context, staged by a resource method (via {@link #stage}) before its
     * manual permission check runs, for the handful of endpoints where the entity's identity lives
     * only in the request body. Not used by endpoints guarded by {@code @PermissionsAllowed}, since
     * that interceptor rejects the request before any resource-method code — including a staging
     * call — could run.
     */
    ...
    /**
     * Holds a staged {@link AuditContext} for the in-flight request. {@code
     * jakarta.ws.rs.container.ContainerRequestContext} is only injectable into JAX-RS providers
     * (filters), not into arbitrary resource method parameters via {@code @Context} — RESTEasy
     * Classic rejects that with {@code RESTEASY003880}. A {@link ThreadLocal} is used instead,
     * which is safe here because RESTEasy Classic's {@code SynchronousDispatcher} invokes the
     * resource method and this filter on the same worker thread within one request.
     */
    private static final ThreadLocal<AuditContext> STAGED_CONTEXT = new ThreadLocal<>();

    /**
     * Stages an {@link AuditContext} for the in-flight request. Call this from a resource method,
     * after parsing the request body enough to know the entity's identity, but before performing a
     * manual (non-{@code @PermissionsAllowed}) permission check — so the context is available
     * regardless of whether that check denies the request.
     */
    public static void stage(AuditContext context) {
        STAGED_CONTEXT.set(context);
    }

    private static final Map<Class<?>, AuditEntityType> RESOURCE_CLASS_TO_ENTITY_TYPE =
            Map.ofEntries(
    ...
    /**
     * The path parameter that names an entity's own ID, for types resolved generically via path
     * params.
     */
    private static final Map<AuditEntityType, String> ENTITY_TYPE_TO_ID_PARAM =
            Map.ofEntries(
    ...
    private void recordIfMutating(
        ...
        AuditLogEntry entry =
                new AuditLogEntry.AuditLogEntryBuilder()
    ...
    /**
     * The generic resolution path used by every resource class with a simple 1:1 {@link
     * AuditEntityType} mapping (i.e. everything except {@link ControlResource} and {@code
     * MappingControllerResource}, which have multiple entity shapes per class).
     */
    private AuditContext resolveGeneric(
            AuditEntityType entityType,
            UriInfo uriInfo,
            String method,
            AuditOutcome outcome,
        ...
        String pathEntityId =
                idParam == null ? null : uriInfo.getPathParameters().getFirst(idParam);
        ...
        if ((entityId == null || version == null) && outcome == AuditOutcome.SUCCESS) {
            LocationSegmentParser.LocationIds ids =
                    LocationSegmentParser.parse(entityType, locationPath(responseContext));
            if (entityId == null) {
    ...
    private AuditContext resolveControlResource(
            String methodName,
            UriInfo uriInfo,
            AuditOutcome outcome,
            ContainerResponseContext responseContext) {
        ...
        return switch (methodName) {
            case "createControlForDomain" ->
                    resolveWithLocationFallback(
                            AuditEntityType.CONTROL_REQUIREMENT, domain, outcome, responseContext);
            case "createRequirementForVersion" ->
                    new AuditContext(
                            AuditEntityType.CONTROL_REQUIREMENT,
                            AuditAction.UPDATE,
                            null,
                            domain,
                            uriInfo.getPathParameters().getFirst("controlId"),
                            version);
            case "createControlConfiguration" ->
                    resolveWithLocationFallback(
                            AuditEntityType.CONTROL_CONFIGURATION,
                            domain,
                            outcome,
                            responseContext);
            case "createConfigurationForVersion" ->
                    new AuditContext(
                            AuditEntityType.CONTROL_CONFIGURATION,
                            AuditAction.UPDATE,
                            null,
                            domain,
                            uriInfo.getPathParameters().getFirst("configId"),
                            version);
            case "deleteControlRequirement" ->
                    new AuditContext(
                            AuditEntityType.CONTROL_REQUIREMENT,
                            AuditAction.DELETE,
                            null,
                            ...
                            uriInfo.getPathParameters().getFirst("controlId"),
                            null);
            case "deleteControlConfiguration" ->
                    new AuditContext(
                            AuditEntityType.CONTROL_CONFIGURATION,
                            AuditAction.DELETE,
                            null,
    ...
    private AuditContext resolveMappingController(
            String methodName,
            UriInfo uriInfo,
            AuditOutcome outcome,
            ContainerResponseContext responseContext) {
        ...
        return switch (methodName) {
            ...
            case "createResourceVersion" -> {
                AuditEntityType entityType =
                        resourceTypeToEntityType(uriInfo.getPathParameters().getFirst("type"));
            ...
            case "createRequirementVersion" ->
                    new AuditContext(
                            AuditEntityType.CONTROL_REQUIREMENT,
                            AuditAction.UPDATE,
                            null,
                            domain,
                            uriInfo.getPathParameters().getFirst("controlName"),
                            version);
            case "createConfigurationVersion" ->
                    new AuditContext(
                            ...
                            null,
                            domain,
                            uriInfo.getPathParameters().getFirst("configName"),
                            version);
            default -> null;
        };
    }
    ...
    /**
     * Resolution for the small set of server-generated-ID CREATE endpoints whose entity isn't
     * namespace-scoped and has no known ID until the store call succeeds (domain creation, initial
     * control creation). Every real call site is a CREATE with no namespace and no already-known
     * entity ID — those aren't parameters because nothing ever varies them; only {@code entityType}
     * and {@code domain} do.
     */
    private AuditContext resolveWithLocationFallback(
            AuditEntityType entityType,
            String domain,
            AuditOutcome outcome,
            ContainerResponseContext responseContext) {
    ...
    /**
     * Maps the {@code {type}} path segment used by MappingControllerResource's name-based endpoints
     * (e.g. {@code "patterns"}) to an {@link AuditEntityType}, reusing {@link
     * org.finos.calm.resources.CalmDocumentParser#TYPE_MAP} rather than duplicating its
     * plural-segment-to-type parsing.
     */
    ...
    private static String locationPath(ContainerResponseContext responseContext) {
        ...
        try {
            ...
            LOG.debug(
                    "Could not parse Location header [{}] for audit logging",
                    STRICT_SANITIZATION_POLICY.sanitize(location.toString()),
                    e);
            return null;
        }
    }
