...
/**
 * Provides atomically-incrementing sequence counters for generating unique integer IDs across all
 * entity types (architectures, patterns, flows, controls, etc.).
 *
 * <h2>How it works</h2>
 *
 * Each entity type has a dedicated counter document in the {@code counters} collection, identified
 * by its {@code _id} (e.g. {@code "architectureStoreCounter"}). The {@link #nextValueForCounter}
 * method uses MongoDB's atomic {@code findOneAndUpdate} with {@code $inc} to increment the counter
 * and return the new value in a single round-trip. Because this is a single atomic operation, no
 * two callers can ever receive the same sequence number — even under high concurrency.
 *
 * <h2>Why no unique index is needed</h2>
 *
 * Unlike the entity collections, the counters collection does not need a unique index because
 * MongoDB's {@code findOneAndUpdate} is inherently atomic. The {@code upsert: true} option ensures
 * the counter document is created automatically on first use.
 *
 * <h2>Failure behaviour</h2>
 *
 * If MongoDB is unavailable, the {@code findOneAndUpdate} call will throw a {@link
 * com.mongodb.MongoException}, preventing any entity from being created with an invalid or
 * duplicate ID.
 */
...
public class MongoCounterStore {
    ...
    public int getNextDocumentSequenceValue() {
        return nextValueForCounter("documentStoreCounter");
    }
    ...
    /**
     * Atomically increments and returns the next sequence value for the given counter. Uses {@code
     * findOneAndUpdate} with {@code $inc} — a single atomic MongoDB operation that guarantees no
     * two callers receive the same value. The {@code upsert: true} option creates the counter
     * document on first use (starting at 1).
     *
