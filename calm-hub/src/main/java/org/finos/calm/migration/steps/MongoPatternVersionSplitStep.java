...
/**
 * Splits the {@code patterns} collection from one document per namespace into one
 * <em>header</em> document per pattern plus one <em>version</em> document per version in the
 * new {@code patternVersions} collection.
 *
 * <p>Version 3 → 4 of the schema, and the Pattern slice of
 ...
 * is immutable: a deployment already past version 3 never re-runs it, so changing it would
...
@ApplicationScoped
public class MongoPatternVersionSplitStep implements SchemaMigrationStep {

    private static final Logger LOG = LoggerFactory.getLogger(MongoPatternVersionSplitStep.class);
    ...
    public MongoPatternVersionSplitStep(MongoDatabase database) {
        this.migration = new MongoVersionSplitMigration(
                database, "patterns", "patternVersions", "patternId", "patterns", "Pattern");
    }
    ...
    public void apply() {
        if (!"mongo".equals(databaseMode)) {
            LOG.info("Skipping pattern version split (database mode: {})", databaseMode);
            return;
        }
        migrate();
    }
