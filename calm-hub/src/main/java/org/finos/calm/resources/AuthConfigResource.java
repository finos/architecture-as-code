...
/**
 * Serves OIDC and GitHub configuration to the frontend at startup.
...
public class AuthConfigResource {
    ...
    Optional<String> githubOAuthClientId;
    ...
    public Response getAuthConfig() {
        ...
        boolean githubEnabled = "github".equals(databaseMode) && githubOAuthClientId.isPresent() && !githubOAuthClientId.get().isBlank();
        Map<String, Object> github = new HashMap<>();
        github.put("enabled", githubEnabled);
        if (githubEnabled) {
            github.put("oauthClientId", githubOAuthClientId.get());
        }
