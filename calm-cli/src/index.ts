...
program
  ...
  .requiredOption(
    PATTERN_OPTION,
    "Path to the pattern file to use. May be a file path or a URL."
  )
  .requiredOption(
    OUTPUT_OPTION,
    "Path location at which to output the generated file.",
    "architecture.json"
  )
  .option(
    SCHEMAS_OPTION,
    "Path to the directory containing the meta schemas to use."
  )
  .option(VERBOSE_OPTION, "Enable verbose logging.", false)
  .option(
    GENERATE_ALL_OPTION,
    'Generate all properties, ignoring the "required" field.',
    false
  )
  .action(
    async (options) =>
...
program
  ...
  .option(
    ARCHITECTURE_OPTION,
    "Path to the architecture file to use. May be a file path or a URL."
  )
  .option(
    SCHEMAS_OPTION,
    "Path to the directory containing the meta schemas to use.",
  ...
  .option(
    STRICT_OPTION,
    "When run in strict mode, the CLI will fail if any warnings are reported.",
    false
  )
  .addOption(
...
async function runValidate(options) {
  if (!options.pattern && !options.architecture) {
    program.error(
      `error: one of the required options '${PATTERN_OPTION}' or '${ARCHITECTURE_OPTION}' was not specified`
    );
  }
  try {
