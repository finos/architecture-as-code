...
describe("CLI Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    await callNpxFunction(`${projectRoot}/../..`, "link calm-cli"); // Link the CLI package to the top-level node_modules
  }, millisPerSecond * 20);
  ...
  test("example validate command - outputting JSON to stdout", (done) => {
    const exampleValidateCommand =
      "calm validate -p ../calm/pattern/api-gateway.json -a ../calm/samples/api-gateway-architecture.json";
    exec(exampleValidateCommand, (_error, stdout, _stderr) => {
      const parsedOutput = JSON.parse(stdout);
      const expectedFilePath = path.join(
        __dirname,
        "../test_fixtures/validate_output.json"
      );
  ...
  test("example validate command - outputting JUNIT to stdout", (done) => {
    const exampleValidateCommand =
      "calm validate -p ../calm/pattern/api-gateway.json -a ../calm/samples/api-gateway-architecture.json -f junit";
  ...
  test("example generate command - does it give the output we expect", (done) => {
    ...
    exec(exampleGenerateCommand, async (_error, _stdout, _stderr) => {
      ...
      const expectedFilePath = path.join(
        __dirname,
        "../test_fixtures/generate_output.json"
      );
      const expectedJson = JSON.parse(
        fs.readFileSync(expectedFilePath, "utf-8")
      );
  ...
  test("example validate command - outputting PRETTY to stdout", (done) => {
    const exampleValidateCommand =
      "calm validate -p ../calm/pattern/api-gateway.json -a ../calm/samples/api-gateway-architecture.json -f pretty";
    exec(exampleValidateCommand, (_error, stdout, _stderr) => {
      const expectedFilePath = path.join(
        __dirname,
        "../test_fixtures/validate_output_pretty.txt"
      );
  ...
  test("example validate command - validates an architecture only", (done) => {
    const calmValidateArchitectureOnlyCommand =
      "calm validate -a ../calm/samples/api-gateway-architecture.json";
    exec(calmValidateArchitectureOnlyCommand, (error, stdout, _stderr) => {
      const expectedFilePath = path.join(
        __dirname,
        "../test_fixtures/validate_architecture_only_output.json"
      );
      const expectedOutput = fs.readFileSync(expectedFilePath, "utf-8");
  ...
  test("example server command - starts server and responds to requests", async () => {
    ...
    const serverCommand =
      "calm server -p 3001 --schemaDirectory ../../dist/calm/";
