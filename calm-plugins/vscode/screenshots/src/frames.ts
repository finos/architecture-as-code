...
// Returns the outer wrapper frame for a webview matching `nameHint`, or undefined
// if not found. `nameHint` is matched case-insensitively against the frame URL —
// pass something distinctive about the panel you want (e.g. 'preview').
//
// Webview wrapper URLs don't actually contain the extension's panel name, so for
// the single-webview case (only our preview is open) just call without a hint.
