---
id: calm-example-index
title: Working with CALM
sidebar_position: 2
---

A hands-on tutorial demonstrating AI-assisted CALM architecture modeling for a hypothetical trading system, an event-driven microservices platform for financial services.

Tutorial Structure
- [Business Context](business-context) - Business requirements for the trading system (trading operations, accounts, positions, workflows)
- [Enable AI Support](enable-ai-support) - Setup GitHub Copilot for CALM (calm init-ai -p copilot)
- [Define Initial Architecture](define-initial-architecture) - Core tutorial: AI identifies nodes from business context → architect refines → AI generates validated architecture JSON with nodes, relationships, and flows
- [Enhance Architecture Content](enhance-architecture-definition) - Placeholder (controls/governance)
- [Generate Documentation](generate-documentation) - Placeholder (template-based docs)
