...
function getUniqueCALMContainers(relationships: CALMRelationship[]) {
    const deployedInContainers: string[] = relationships
        .filter(
    ...
    const composedOfContainers: string[] = relationships
        .filter(
...
function createGraphRelationship(
    ...
    const deployedInandComposedOfRelationships = relationships.filter(
        (relationship) =>
            'deployed-in' in relationship['relationship-type'] ||
            'composed-of' in relationship['relationship-type']
    )

    const containers = getUniqueCALMContainers(
        deployedInandComposedOfRelationships
    )
    ...
    relationships.forEach((relationship) => {
        if ('connects' in relationship['relationship-type']) {
            ...
            createDeployedInRelationship(
                instance,
                relationship as CALMDeployedInRelationship
            )
        } else {
            createComposedOfRelationship(
                instance,
                relationship as CALMComposedOfRelationship
            )
        }
    })
}
