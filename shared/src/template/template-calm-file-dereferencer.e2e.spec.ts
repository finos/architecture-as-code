...
describe('End-to-End Dereferencing Test with Per-Test Mapping', () => {
    const basePath = path.resolve(__dirname, '../../../calm/samples/2024-12/traderx');
    let traderxJson: string;

    beforeAll(() => {
        traderxJson = fs.readFileSync(path.join(basePath, 'traderx.json'), 'utf-8');
    });
    ...
    it('fully dereferences when all flows are mapped', async () => {
        ...
        const dereferencer = new TemplateCalmFileDereferencer(urlFileMapping, new CompositeReferenceResolver());
    ...
    it('dereferences only the mapped flows and leaves others untouched', async () => {
        ...
        const inlined = parsed.flows.filter((f: object) => typeof f === 'object' && f['unique-id']);
