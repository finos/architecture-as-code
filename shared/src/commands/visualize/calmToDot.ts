...
function addRelationship(g: Digraph, relationship: CALMRelationship): void {
  logger.debug(
    `Using the following relationship [${JSON.stringify(relationship)}]`
  );
...
function addConnectsRelationship(
  g: Digraph,
  ...
  logger.debug(
    `Creating a connects relationship from [${sourceId}] to [${destinationId}]`
  );

  g.addEdge(
    new Edge([getNode(sourceId), getNode(destinationId)], {
...
function addInteractsRelationship(
  g: Digraph,
  ...
  targetIds.forEach((maybeId) => {
    ...
    g.addEdge(
      new Edge([getNode(sourceId), getNode(targetId)], {
...
function addDeployedInRelationship(
  g: Digraph,
  relationship: CALMDeployedInRelationship
) {
  const containerId =
  ...
  targetIds.forEach((targetId) => {
    logger.debug(
      `Creating a deployed-in relationship from [${containerId}] to [${targetId}]`
    );
...
function addComposedOfRelationship(
  g: Digraph,
  relationship: CALMComposedOfRelationship
) {
  const containerId =
  ...
  targetIds.forEach((targetId) => {
    logger.debug(
      `Creating a composed-of relationship from [${containerId}] to [${targetId}]`
    );
