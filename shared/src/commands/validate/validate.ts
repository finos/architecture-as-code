 ...
 */
function mergeSpectralResults(
  spectralResultPattern: SpectralResult,
  spectralResultArchitecture: SpectralResult
): SpectralResult {
  const errors: boolean =
    spectralResultPattern.errors || spectralResultArchitecture.errors;
  const warnings: boolean =
    spectralResultPattern.warnings || spectralResultArchitecture.warnings;
  const spectralValidations = spectralResultPattern.spectralIssues.concat(
    spectralResultArchitecture.spectralIssues
  );
 ...
 */
export function exitBasedOffOfValidationOutcome(
...
async function loadMetaSchemas(ajv: Ajv2020, metaSchemaLocation: string) {
  ...
  if (isValidURL(metaSchemaLocation)) {
    ...
    if (!statSync(metaSchemaLocation).isDirectory()) {
      throw new Error(
        `The metaSchemaLocation: ${metaSchemaLocation} is not a directory`
      );
    }
    ...
    if (filenames.length === 0) {
      throw new Error(
        `The metaSchemaLocation: ${metaSchemaLocation} is an empty directory`
      );
    }
    ...
    filenames.forEach((filename) => {
      if (filename.endsWith(".json")) {
        ...
        const meta = JSON.parse(
          readFileSync(metaSchemaLocation + "/" + filename, "utf8")
        );
...
export function sortSpectralIssueBySeverity(
  issues: ISpectralDiagnostic[]
): void {
  issues.sort(
    (issue1: ISpectralDiagnostic, issue2: ISpectralDiagnostic) =>
      issue1.severity.valueOf() - issue2.severity.valueOf()
  );
}

export function convertJsonSchemaIssuesToValidationOutputs(
  jsonSchemaIssues: ErrorObject[]
): ValidationOutput[] {
  return jsonSchemaIssues.map(
    (issue) =>
...
export function convertSpectralDiagnosticToValidationOutputs(
  spectralIssues: ISpectralDiagnostic[]
): ValidationOutput[] {
...
async function loadFileFromUrl(fileUrl: string) {
  ...
  if (!res.ok) {
    throw new Error(
      `The http request to ${fileUrl} did not succeed. Status code ${res.status}`
    );
  }
...
export async function validateAndExitConditionally(
  ...
  const outcome = await validate(
    jsonSchemaArchitectureLocation,
    jsonSchemaLocation,
    metaSchemaPath,
    debug
  );
...
export async function validate(
  ...
  try {
    ...
    const spectralResultForPattern: SpectralResult =
      await runSpectralValidations(
        stripRefs(jsonSchema),
    ...
    logger.info(
      `Loading architecture from : ${jsonSchemaArchitectureLocation}`
    );
    const jsonSchemaArchitecture = await getFileFromUrlOrPath(
      jsonSchemaArchitectureLocation
    );

    const spectralResultForArchitecture: SpectralResult =
      await runSpectralValidations(
        jsonSchemaArchitecture,
        validationRulesForArchitecture
      );

    const spectralResult = mergeSpectralResults(
      spectralResultForPattern,
      spectralResultForArchitecture
    );
    ...
    if (!validateSchema(jsonSchemaArchitecture)) {
      logger.debug(
        `JSON Schema validation raw output: ${prettifyJson(
          validateSchema.errors
        )}`
      );
      errors = true;
      jsonSchemaValidations = convertJsonSchemaIssuesToValidationOutputs(
        validateSchema.errors
      );
    }
 ...
 */
function validatePatternOnly(
  spectralValidationResults: SpectralResult,
  patternSchema: object,
  ajv: Ajv2020
): ValidationOutcome {
  logger.debug(
    "Architecture was not provided, only the JSON Schema will be validated"
  );
  ...
  try {
    ...
    errors = true;
    jsonSchemaErrors.push(
      new ValidationOutput("json-schema", "error", error.message, "/")
    );
  }
...
function extractSpectralRuleNames(): string[] {
  const architectureRuleNames = getRuleNamesFromRuleset(
    validationRulesForArchitecture
  );
